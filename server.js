...
const mime={'.txt':'text/plain; charset=utf-8','.html':'text/html','.css':'text/css','.js':'application/javascript','.mjs':'application/javascript','.webp':'image/webp','.png':'image/png','.jpg':'image/jpeg','.jpeg':'image/jpeg','.svg':'image/svg+xml','.ico':'image/x-icon','.json':'application/json'};
...
const server=http.createServer(async(req,res)=>{
  ...
  const asset=/^assets\/[a-zA-Z0-9_.-]+\.(webp|png|jpg|jpeg|svg|ico|js|mjs)$/.test(relative)||/^assets\/samples\/[a-zA-Z0-9_-]+\.txt$/.test(relative);
