...
const RelationshipIntelligence=(()=>{
 ...
 function escRe(s){return String(s).replace(/[.*+?^${}()|[\]\\]/g,'\\function namesIn(sentence,characters){const low=sentence.toLowerCase();return characters.filter(c=>[c.canonicalName,...(c.aliases||[])].some(n=>low.includes(String(n).toLowerCase()))).map(c=>c.canonicalName);}');}
